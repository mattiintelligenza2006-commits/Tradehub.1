  ...
  return `
    <div class="news-card">
      ...
      <div class="news-meta">
        ...
        <span class="badge ${item.verified ? 'badge-verified' : 'badge-unverified'}">
          ${item.verified ? '✓ Verificato' : '⚠ Non Verificato'}
        </span>
        <span class="cross-verify">
          <i data-lucide="shield-check" style="width:12px;height:12px"></i>
          ${item.crossVerify} fonti
        </span>
...
function renderNews() {
  document.getElementById('newsGlobal').innerHTML = NEWS_DATA.global.map(renderNewsCard).join('');
  document.getElementById('newsCommodities').innerHTML = NEWS_DATA.commodities.map(renderNewsCard).join('');
  document.getElementById('newsCrypto').innerHTML = NEWS_DATA.crypto.map(renderNewsCard).join('');
  document.getElementById('newsCentralBanks').innerHTML = NEWS_DATA.centralBanks.map(renderNewsCard).join('');
  lucide.createIcons();
}
...
function renderAssets() {
  ...
  grid.innerHTML = ASSETS_DATA.map((a, idx) => {
    ...
    return `
      <div class="asset-card ${isPrimary ? 'primary' : ''}" id="asset-card-${idx}">
